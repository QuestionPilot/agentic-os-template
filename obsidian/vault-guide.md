...
## 10. Evaluated and Deliberately Not Shipped

Two vault tools were reviewed for promotion into this template and rejected on their merits. They are recorded here so a future contributor does not re-derive — or re-propose — them without the context.

- **A recall-eval runner (lesson-index trigger measurement).** Rejected as drafted for two independent reasons: its runner hardcoded a single harness's CLI invocation, which `skills/skill-authoring.md` forbids by name (harness-native runners must not hardcode one harness), and its fixture corpus tested one operator's private tool vocabulary rather than recall quality in general. The salvageable pieces — the fixture *format*, the before/after re-phrase protocol, a positive control ("every index row matches at least one fixture"), and synthetic examples — ship via the scaffold's retrieval-fixture contract instead. A trigger-measurement layer remains worth building, but only on top of a harness-neutral inference adapter, which does not exist yet.
- **A machine-bootstrap backup script (copying local config and agent memory stores into the vault).** Rejected because it contradicts two contracts this template ships: `core/memory-model.md` § Cache Contract ("Never sync caches machine-to-machine" — per-machine agent memory stores are caches rebuilt from the vault and tracker) and this guide's §5 exclusion of machine-specific config from the vault. Copying `local.env`-class files into a cloud-synced vault also moves auth-adjacent, PII-bearing state onto a sync surface the vault's threat model excludes. Path cleanup does not fix this; it is the wrong shape, not a portability defect. An operator who wants that backup keeps it as a private, vault-local tool — the template will not ship it.
