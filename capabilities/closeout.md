...
Conditional depth — the why, the case studies, and the drain's identity and
body-shape rationale — lives in `$AI_CONFIG_DIR/capabilities/reference/closeout.md`.
Every must-fire gate is inline below.
...
or when a meaningful workstream finished.
...
2. Note any active Linear issue; do not invent one. If this session's work
   completed it, move it to Done as part of closeout — verify, never assume an
   integration did it.
...
4. **Enumerate State Deltas.** Linear projects created / closed / status-changed
   ...
   ones); new durable on-disk artifact directories.
   **Reactivation counts, and its trigger is operational, not a tracker status
   transition:** this session filed or picked up an issue under a project with no
   live project-type memory note, so the project is active again with no note —
   recreate the note at THIS closeout, because a noteless active project is
   invisible to the next orient's body-read step. One event may satisfy several
   triggers — write one consolidated `state-delta` lesson per event. Each delta's
   memory write happens BEFORE session exit, never deferred. Canonical taxonomy:
...
0. **EAD gate (Eliminate / Automate / Delegate):** What did we build this session
   ...
   as `no-action`. Q0 skips only the lesson-classification walk (questions 1–6) —
   not the State Deltas, file sweep, Q7a checks, Linear updates, or the output block.
   State-delta lessons remain mandatory regardless of Q0 outcome, written during
   closeout itself. The Q7 file sweep below ALSO remains mandatory regardless of Q0
   outcome, and so is Q7a verification (operator-main git-state cleanliness) — junk
   files and a stray staged change survive a `no-action` close just as readily as a
   substantive one.
...
3. Can the lesson become a check or script instead of prose? **And — did this
   ...
   contract in `$AI_CONFIG_DIR/skills/skill-authoring.md` (principle 11). Its
   fixture-test and explicit-user-approval steps are non-optional; never
   auto-promote an unvetted flow into the trusted skill set.
   ...
   **Q3b — golden-session extraction (positive-lesson capture).** Was any deliverable
   ...
   about, and how it was proven) and classify it through the table below like any
   other lesson, exactly one class. Write boundary: the Q3a candidate row is the
   ONLY direct write on this path — every other landing is proposal-only (propose
   path + body, explicit user approval before any write). A method that fires both
   Q3a and Q3b is one lesson — log it once.
4. Does Linear need updating — a completed issue moved to Done, a status corrected,
   ...
   metadata at create time plus the structured body. The issue's `state-delta`
   line records it as created
   `to-standard` (a draft-only session with no write surface records
   ...
   **Q4a — project-note closeout edit.** If a Linear project transitioned to
   ...
   reverse transition carries the mirror duty: on the reactivation trigger above, the
   recreated note is part of THIS closeout, and a vault project note flipped to a
   terminal state at the earlier close flips back to active in the same pass.
...
7. **File sweep:** What files / directories did this session create that should NOT
   ...
   `Bash cp` / shell redirect / tracker-CLI file output the session performed.
   Attach exactly one explicit classification to each:
   - `keep-because-<reason>` — durable evidence to preserve; state the reason, never
     leave it bare.
   ...
   - `clean-by-<date-or-owner>` — survives this session on a bounded lifetime; name
     the retention rule explicitly so a future hygiene sweep collects it without
...
Canonical wording: `$AI_CONFIG_DIR/core/self-improvement.md` — if this body and that
file diverge, that file wins; fix the divergence rather than following it.

## Lesson classification

For each lesson, pick exactly one class and route it. Class definitions:
`$AI_CONFIG_DIR/core/self-improvement.md`.
...
| `linear` | A Linear issue | Create or comment; reference the closeout summary. A NEW issue conforms to the Q4 standard. |
| `obsidian` | The durable knowledge base | Lesson distilled FRESH from this session: propose path + body, do not write. A `feedback_*` / `decision` note already in the native store: promote it into `04-Lessons` per the distillation steps below (a cache→durable write). |
| `playbook` | `$AI_CONFIG_DIR/playbooks/*.md` | New or updated workflow file (requires user approval). |
| `skill` | The harness's capability/skill set | Promote a successful repeatable flow via the seven-step trust contract in `$AI_CONFIG_DIR/skills/skill-authoring.md` (principle 11). |
...
| `state-delta` | a project-type memory note (`metadata.type: project`, or `runtime_*.md`) + `MEMORY.md` index update | Mandatory when a Linear project or new durable on-disk artifact dir came into existence this session, or a project became active again with no project-type memory note. Write the memory pointer during closeout itself — never deferred. |
...
- **State Deltas** — required whenever any state changed; `_none_` is the valid empty
  value, and its memory writes happen during closeout, not deferred. Any
...
- **Files created this session** — the Q7 sweep output, one line per artifact with its
  explicit classification. Anything classified `clean-now` is removed INLINE before
  the block emits and written here as `cleaned-now`.
- **Pick up here** — the final line, one sentence naming the next concrete action. If
  the work is fully complete, write `done` and link the artifact (PR URL, Linear
...
**Pre-flight — before writing:** confirm `$OBSIDIAN_VAULT_PATH` exists and is fully
synced down (missing or mid-sync → FLAG, never write a half-state); scan for provider
conflict copies (`foo (1).md`, names containing `conflict`) and resolve or FLAG before
adding more. Closeout is operator-serialized — one machine at a time.
...
bare basename. Both rules are enforced by the pre-write gate below; a violation fails
closed.
...
project-note budget — and returns one verdict. Exit 0 = every applicable check passed
→ write. **Non-zero = do NOT write**: a check failed, or a check's script is missing
(a gate that cannot run has proven nothing, so it fails rather than skips). Remediate
what the run names, then re-run. An absent target surface (no vault, no memory dir) is
a NAMED SKIP, not a failure. The script header documents each check and the
fail-closed contract.

After writing, **confirm the file exists** at the target path. If it does not,
surface a **FLAG** in the closeout output and record the miss — never report a silent
success.
...
- The transcript is the marker that closeout ran — do not write a separate
  ...
  *content*.)
- `closeout` is manual-fire (see "When to invoke"): the discipline lives in the
  session-agent kickoff orientation and the agent/operator remembering, not a gate.
