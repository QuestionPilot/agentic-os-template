...
3. Can the lesson become a check or script instead of prose?
   ...
   **Q3b — golden-session extraction (positive-lesson capture).** Was any deliverable this session *unusually good* — a one-off result that came out notably better than usual? This is the positive twin of Q1a's recall-failure capture and complements Q3a's repeated-procedure capture: the trigger here is quality, not repetition or failure. If yes, mine the session for the *method* behind it — what was thought about (the framing, the decomposition, the checks that were weighed) and how it was proven (the evidence that made the result trustworthy) — and land that method in an EXISTING class as a candidate, never an autonomous write. **Where extracted methods land:** a `skill` candidate via the Q3a candidate-row mechanism (the catalog's candidates section) when the method is a repeatable procedure; a `playbook` proposal (a new/updated `playbooks/*.md` workflow file, requires explicit user approval) when it is a workflow; or, if it fits neither, an `obsidian` lesson (propose-don't-write). Positive lessons strand more easily than failures because nothing forces them to surface, so this is the capture gate — naming the method is what lets a future session reproduce the result deliberately instead of by luck.
