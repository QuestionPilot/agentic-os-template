...
- The framework's bootstrap scripts do NOT use `curl | sh`, and neither do its setup docs. The one third-party CLI the framework documents installing — the optional `linear` CLI (`linear/linear-setup.md` §3.2) — is installed by `scripts/install-linear-cli.sh` (or the `.ps1` twin): a pinned release tag, its archive verified against the operator-reviewed sha256 pins in `scripts/linear-cli-checksums.sha256` before extraction, the extracted binary re-verified in place before it is ever executed, followed by a version smoke. An unpinned tag is refused, not downloaded on trust. (The deprecated `install-lineark.sh` installer for the previous CLI surface follows the same shape and remains for one transition release.) Any *other* installer you reach for from adjacent documentation is out of band — audit it before running it, exactly as you would audit any third-party installer.
