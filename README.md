...
- Hook scripts (`harnesses/<h>/hooks/*.sh`, each with a `.ps1` twin) run as your shell user on Claude Code / Codex session events. They never `eval` operator-supplied content: every hook reads its event JSON from stdin and extracts fields with `jq -r` as data. Specifically: the gate hook (`session-agent.sh`, PreToolUse on Write/Edit/NotebookEdit) extracts `transcript_path` and `session_id`, then reads the transcript as data through `grep` / `jq -c` / `awk` / `sed` — no command execution from event content; it **fails closed** (denies the edit) when `jq` is absent. The surfacing hook (`framework-surface.sh`, SessionStart) reads `.source` and `.session_id` (sanitized before it is used in a marker-file path), runs `git -C "$AI_CONFIG_DIR" log`, `claude mcp list`, and the freshness / orphaned-local-hook / distillation-lag checkers, parsing all of it as untrusted text; it **fails open** (silent), and emits its whole payload once at the end — a hook-timeout kill loses all of it, so its sub-checks are bounded with `timeout` / `gtimeout` where one is on PATH. The stuck detector (`stuck-detector.sh`, PostToolUse + PostToolUseFailure on Bash) hashes the failed command text and keeps a per-session streak file beside the gate markers (`agentic-os/stuck-<session_id>`, reaped after 7 days); it never runs or re-runs the command. Kill switches per hook: `CLAUDE_SKIP_SESSION_AGENT=1` (gate), `CLAUDE_SKIP_FRAMEWORK_SURFACE=1` (whole surfacing hook) plus its per-block switches `CLAUDE_SKIP_FRESHNESS_CHECK`, `CLAUDE_SKIP_LOCAL_HOOK_CHECK`, `CLAUDE_SKIP_MCP_PROBE`, `CLAUDE_SKIP_DISTILLATION_NUDGE`, `CLAUDE_SKIP_SESSION_AGENT_DIRECTIVE`, and `CLAUDE_SKIP_STUCK_DETECTOR=1`. (The `closeout` Stop hook and its `CLAUDE_SKIP_CLOSEOUT` switch were removed; closeout is manual-fire.)
...
- A clean git tree is a claim about repository content only. It does not cover gitignored harness homes such as `$CODEX_HOME`, including Codex's user-owned `config.toml`. Before a clean-footprint claim that includes a Codex run, use `bash scripts/check-codex-trust.sh` or `pwsh -NoProfile -File scripts/check-codex-trust.ps1` with `CODEX_HOME` set, or pass `--config /path/to/config.toml`; an unset `CODEX_HOME` without `--config` fails closed. The read-only check reports trusted-project count and paths, and it fails if the TOML parser or config is unavailable or malformed. Exit 0 means only that the inventory was parsed and reported; it does not mean there are no trusted rows or that permissions are safe. For per-run evidence, compare inventories from before and after the run; one inventory cannot attribute a row to that run. Use `codex exec -s read-only …` for a fresh launch. Use `codex exec resume -c 'sandbox_mode="read-only"' …` for a resume; that command does not accept `-s`. Do not rely on a saved project trust row.
- `.agents/skills/` is a shared project skill surface, not Gemini-only. Hermes v0.21.3 loads it from a trusted project before profile skills. When rendering Hermes from this framework checkout, the installer asks the selected profile's Hermes resolver whether the checkout is trusted and refuses confirmed divergent same-name shadows without changing trust. If that resolver is unavailable, it warns that the trust state was not inspected; it does not claim safety.

**Adjacent install instructions are out of band:**

- The framework's bootstrap scripts do NOT use `curl | sh`, and neither do its setup docs. The one third-party CLI the framework documents installing — the optional `linear` CLI (`linear/linear-setup.md` §3.2) — is installed by `scripts/install-linear-cli.sh` (or the `.ps1` twin): a pinned release tag, its archive verified against the operator-reviewed sha256 pins in `scripts/linear-cli-checksums.sha256` before extraction, the extracted binary re-verified in place before it is ever executed, followed by a version smoke. An unpinned tag is refused, not downloaded on trust. Any *other* installer you reach for from adjacent documentation is out of band — audit it before running it, exactly as you would audit any third-party installer.
