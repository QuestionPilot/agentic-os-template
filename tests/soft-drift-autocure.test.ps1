...
_Skip 't-106.test: --cure-soft-drift cures soft keys (theme + effortLevel)' $reason106cure
_Skip "t-106.test: post-cure: operator theme preserved as 'auto'" $reason106cure
_Skip "t-106.test: post-cure: operator effortLevel preserved as 'xhigh'" $reason106cure
_Skip 't-106.test: post-cure: drift check passes from scratch' $reason106cure
...
_Skip 't-106.test: preserve-live recorded operator effortLevel before strip' $reason106cure
_Skip 't-106.test: app-strip of operator soft keys cures (soft envelope)' $reason106cure
_Skip 't-106.test: post-cure: stripped theme not resurrected (operator-local)' $reason106cure
_Skip 't-106.test: post-cure: stripped effortLevel not resurrected (operator-local)' $reason106cure
_Skip 't-106.test: post-cure: drift check passes after app-strip cure' $reason106cure
...
_Skip 't-106.test: --cure-soft-drift absorbs app-written notification keys' $reason106cure
_Skip 't-106.test: post-cure: agentPushNotifEnabled preserved through cure re-render' $reason106cure
...
_Skip 't-106.test: --cure-soft-drift accepts flag BEFORE --manifest' $reason106cure
...
# DEFERRED — same CRLF cause as Test 2 above.
_Skip 't-106.test: extraKnownMarketplaces key reorder accepted as soft drift' $reason106cure

# ---------- Test 14: enabledPlugins value-add is NOT silently cured ---------
# DEFERRED — same CRLF cause as Test 2 above (check-drift.ps1 cure path).
_Skip 't-106.test: enabledPlugins value-add is NOT cured by --cure-soft-drift (refused)' $reason106cure
_Skip 't-106.test: post-refusal: injected plugin still flagged by default drift check' $reason106cure
_Skip 't-106.test: post-refusal: injected plugin still present in live settings.json' $reason106cure
...
if ($inGitRepo) {
    ...
    if ($TOPLEVEL -and $MAIN_ROOT_EXPECTED -and ($TOPLEVEL -ne $MAIN_ROOT_EXPECTED)) {
        # DEFERRED — same CRLF cause as Test 2 above.
        _Skip 't-106.test: worktree case: --cure-soft-drift sources install.sh from main repo (still cures)' $reason106cure
    } else {
