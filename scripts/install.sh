...
# double-quoted scalar. (The PS twin applies the same layering in
# Get-HermesHookCommandYaml around its `pwsh -NoProfile -File` launcher —
# Hermes's ShellHookSpec takes `command` as ONE string; there is no `args:` key.)
...
generate_hermes_hooks() {
  # A native Windows Hermes cannot execute this render's wiring: install.sh
  # would emit MSYS-spelled paths to .sh hook scripts, which the desktop app
  # can neither resolve nor execute — the hooks silently never fire (no spine
  # auto-fire, no edit gate). install.ps1 is the supported
  # hermes path on Windows: it wires the .ps1 hook twins through a single
  # `pwsh -NoProfile -File` command string. Fail loudly instead of rendering
  # a silently-dead hook wire.
  case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*)
      die "hermes on Windows: use scripts/install.ps1 --harness hermes — install.sh would wire MSYS-path .sh hooks that a native Windows Hermes cannot execute (the spine hooks would silently never fire)" ;;
  esac
