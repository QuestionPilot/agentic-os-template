...
if [ "${#HARNESSES[@]}" -gt 1 ]; then
  ...
  for h in "${HARNESSES[@]}"; do
    tenv="$(harness_target_env "$h")" || die "unknown harness '$h' (known: claude, codex)"
...
target_env="$(harness_target_env "$HARNESS")" \
  || die "unknown harness '$HARNESS' (known: claude, codex)"
...
main() {
  ...
  # framework-surface is a non-capability hook — wired unconditionally.
