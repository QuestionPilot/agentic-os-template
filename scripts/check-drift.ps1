...
.DESCRIPTION
    ...
    2. (default): scan the repo root for portability + denylist violations:
       ...
       - no retired-project camel/spaced markers (the <TEAM>-90 forward-pointer
         allowlist tolerates a single canonical cross-model-review URL only)
...
# ---------------------------------------------------------------------------
# Retired-marker scan with per-occurrence allowlist for the canonical
# cross-model-review forward-pointer URL. Capture hits, then for each line
# strip every occurrence of the allowed URL and re-check for any remaining
# disallowed literal. Closes the line-based-allowlist bypass.
# ---------------------------------------------------------------------------

# Runtime-construct the marker literals so this file doesn't trip its own scan.
# Sentinel halves: each piece is non-matching alone; concatenated at runtime
# gives the literals 'Que' + 'stion' + 'Pilot' / spaced variant. Comments
# AVOID the unbroken literal per [[feedback_self_tripping_test_source]]
# <TEAM>-87 extension (the surrounding source itself is part of the scan
# surface; bash check-drift.sh does NOT exclude check-drift.ps1 yet).
$_qpCamel = 'Que' + 'stion' + 'Pilot'
$_qpSpaced = 'Que' + 'stion' + ' Pilot'
$retiredRe = [regex]::Escape($_qpCamel) + '|' + [regex]::Escape($_qpSpaced)

$retiredHits = Test-ScanPath `
    -Root $repoRoot `
    -ExcludeFiles @(
        'check-drift.sh', 'check-drift.ps1',
        'local.env', '.git',
    ...
    -ExcludeDirs @('.claude', '.codex', '.agents', 'cross-model-out', 'plans') `
    -Pattern $retiredRe

$allowedUrl = 'https://github.com/' + $_qpCamel + '/cross-model-review'
$residue = New-Object System.Collections.Generic.List[string]
foreach ($line in $retiredHits) {
    if ([string]::IsNullOrEmpty($line)) { continue }
    $stripped = $line.Replace($allowedUrl, '')
    if ($stripped -match $retiredRe) {
        [void]$residue.Add($line)
    }
}
if ($residue.Count -gt 0) {
    # Use the bash twin's exact wording so byte-parity holds. Both twins are
    # excluded from each other's scan via --exclude=check-drift.{sh,ps1}.
    Write-Fail "retired project-specific marker found ($_qpCamel / $_qpSpaced)"
    foreach ($h in $residue) { [Console]::Error.WriteLine($h) }
    exit 1
}
