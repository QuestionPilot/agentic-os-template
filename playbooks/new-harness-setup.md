...
7. Confirm the build produced the expected skill files and hook wiring under the harness's config dir (the directory named by the target env var in `local.env` — `CLAUDE_CONFIG_DIR` for Claude, `CODEX_HOME` for Codex, `HERMES_HOME` for Hermes, `CURSOR_CONFIG_DIR` for Cursor). The drift gate `scripts/check-drift.sh --manifest` detects hand-edits to compiled output.
